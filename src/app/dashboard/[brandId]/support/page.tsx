      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1">
          ...
          {tickets.length === 0 ? (
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-12 text-center">
              <HeadphonesIcon className="h-12 w-12 text-slate-300 mx-auto mb-4" />
              <h3 className="font-semibold mb-1">No tickets yet</h3>
              <p className="text-sm text-slate-400">Create a support ticket to get started</p>
            </div>
          ) : (
              ...
              {tickets.map(ticket => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left bg-white dark:bg-slate-800 border rounded-xl p-4 transition-all hover:shadow-md cursor-pointer ${
                      selectedTicket === ticket.id ? 'border-blue-500 shadow-md' : 'border-slate-200 dark:border-slate-700'
                    }`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3 min-w-0">
                        <StatusIcon className="h-5 w-5 text-slate-400 mt-0.5 flex-shrink-0" />
                        <div className="min-w-0">
                          <p className="font-medium text-sm truncate">{ticket.subject}</p>
                          <p className="text-xs text-slate-400 mt-0.5">{ticket.customer_name} · {new Date(ticket.created_at).toLocaleDateString()}</p>
                        </div>
                      </div>
        ...
        {selectedTicket && selectedTicketData && (
          <motion.div
            ...
            className="lg:w-[400px] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden flex flex-col"
