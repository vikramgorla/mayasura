    ...
    <div className="p-8 h-[calc(100vh-0px)] flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Test your brand&apos;s AI-powered customer support</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {messages.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="font-semibold mb-1">Chat with {brand.name}</h3>
              ...
              <div className="flex flex-wrap justify-center gap-2 mt-4">
                {['What do you offer?', 'Tell me about your brand', 'What are your prices?'].map((q) => (
                  <button
                    key={q}
                    onClick={() => { setInput(q); }}
                    className="px-3 py-1.5 rounded-full bg-slate-50 text-slate-600 text-xs hover:bg-slate-100 transition-colors cursor-pointer"
                  >
                    {q}
                  </button>
                ))}
